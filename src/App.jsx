...
// Утилита для безопасной генерации ID
...
export default function App() {
  ...
  // Data States (Синхронизируются с облаком)
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        showToast('Ошибка авторизации в Firebase', 'error');
      }
    };
    initAuth();
  ...
  // 2. Подписка на данные Firestore (Только для текущего юзера)
  useEffect(() => {
    if (!user) return;

    const bikesRef = collection(db, 'artifacts', appId, 'users', user.uid, 'bikes');
    const unsubBikes = onSnapshot(bikesRef, (snapshot) => {
      const loadedBikes = snapshot.docs.map(doc => doc.data());
      setBikes(loadedBikes);
      if (loadedBikes.length > 0 && !activeBikeId) {
        setActiveBikeId(loadedBikes[loadedBikes.length - 1].id);
      }
      setIsLoading(false);
    }, (err) => showToast('Ошибка загрузки гаража', 'error'));

    const compRef = collection(db, 'artifacts', appId, 'users', user.uid, 'components');
    const unsubComp = onSnapshot(compRef, (snapshot) => {
      setComponents(snapshot.docs.map(doc => doc.data()));
    }, (err) => showToast('Ошибка загрузки деталей', 'error'));

    const logsRef = collection(db, 'artifacts', appId, 'users', user.uid, 'logs');
    const unsubLogs = onSnapshot(logsRef, (snapshot) => {
      const logs = snapshot.docs.map(doc => doc.data());
      logs.sort((a, b) => b.id.localeCompare(a.id)); // Сортировка логов (по ID с timestamp)
      setActivityLogs(logs);
    }, (err) => showToast('Ошибка загрузки истории', 'error'));

    return () => { unsubBikes(); unsubComp(); unsubLogs(); };
  }, [user]); // Убрали activeBikeId из зависимостей, грузим все данные юзера (Firestore сам кэширует)

  const activeBike = bikes.find(b => b.id === activeBikeId) || null;

  // Математика износа 2.0 (ПРО-версия с защитой от NaN и прогнозом)
  const activeComponentsWithWear = useMemo(() => {
    ...
    return components
      ...
      .map(comp => {
        const instMil = comp.installMileage ?? 0;
        const distanceSinceInstall = Math.max(0, totalMil - instMil);
  ...
  }, [components, activeBike]); // Зависимость правильно следит за activeBike

  const criticalAlerts = activeComponentsWithWear.filter(c => c.status === 'critical' || c.status === 'warning');

  // --- ХЕЛПЕРЫ ДЛЯ FIREBASE (С обработкой ошибок) ---
  const saveDocSafe = async (collectionName, document) => {
    if (!user) return;
    try {
      await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, collectionName, document.id), document);
    } catch (e) {
      console.error(e);
      showToast(`Ошибка записи в БД (${collectionName})`, 'error');
      throw e;
    }
  };

  const delDocSafe = async (collectionName, documentId) => {
    if (!user) return;
    try {
      await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, collectionName, documentId));
    } catch (e) {
      console.error(e);
      showToast(`Ошибка удаления из БД (${collectionName})`, 'error');
      throw e;
    }
  };
  ...
  const handleAddRide = async (distance, notes = '') => {
    ...
    try {
      const updatedBike = { ...activeBike, totalMileage: (activeBike.totalMileage ?? 0) + distNum };
      await saveDocSafe('bikes', updatedBike);

      const newLog = {
        id: `log_${Date.now()}_${generateId()}`,
        bikeId: activeBike.id,
        date: new Date().toISOString().split('T')[0],
        type: 'ride',
        text: `Заезд на ${distNum} км` + (notes ? `\nЗаметки: ${notes}` : '')
      };
      await saveDocSafe('logs', newLog);
      showToast('Поездка успешно добавлена!');
    } catch (e) {
      // Ошибка уже показана в saveDocSafe
    }
  };
  ...
  const handleQuickReplace = async (oldComponentId, newItemDetails) => {
    ...
    try {
      const archivedComp = { ...oldComp, archived: true, archiveDate: new Date().toISOString().split('T')[0] };
      await saveDocSafe('components', archivedComp);
      ...
      const newComponent = {
        ...
        lifespan: Number(newItemDetails.lifespan) || Number(oldComp.lifespan) || 0,
        archived: false
      };
      await saveDocSafe('components', newComponent);
      ...
      const newLog = {
        ...
        date: new Date().toISOString().split('T')[0],
        type: 'maintenance',
        text: `Замена "${newItemDetails.name || 'узла'}" на пробеге ${Math.round(activeBike.totalMileage ?? 0)} км`
      };
      await saveDocSafe('logs', newLog);
      showToast('Деталь заменена, ресурс сброшен!');
    } catch (e) {
      // handled
    }
  };
  ...
  const handleAddNewManualComponent = async (compData) => {
    ...
    try {
      const newComponent = {
        ...
        lifespan: Number(compData.lifespan) || 0,
        archived: false
      };
      await saveDocSafe('components', newComponent);
      showToast('Новый узел добавлен');
    } catch (e) {}
  };
  ...
  const confirmDeleteBike = async () => {
    ...
    try {
      // Удаляем сам байк
      await delDocSafe('bikes', bikeId);
      
      // Удаляем связанные данные (каскадное удаление на клиенте)
      const compsToDelete = components.filter(c => c.bikeId === bikeId);
      for (let c of compsToDelete) {
        await delDocSafe('components', c.id);
      }
      
      const logsToDelete = activityLogs.filter(l => l.bikeId === bikeId);
      for (let l of logsToDelete) {
        await delDocSafe('logs', l.id);
      }
      
      const remainingBikes = bikes.filter(b => b.id !== bikeId);
      if (activeBikeId === bikeId) {
        setActiveBikeId(remainingBikes.length > 0 ? remainingBikes[0].id : null);
        if (remainingBikes.length === 0) setActiveTab('garage');
      }
      showToast('Велосипед удален');
    } catch (e) {
      // handled
    } finally {
  ...
  const handleAddBike = async (newBike, newComponents) => {
    try {
      await saveDocSafe('bikes', newBike);
      for (const comp of newComponents) {
        await saveDocSafe('components', comp);
      }
      setActiveBikeId(newBike.id);
      setActiveTab('dashboard');
      showToast('Велосипед успешно добавлен в гараж!');
    } catch (e) {}
  };
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="text-center">
          <Loader2 className="w-10 h-10 md:w-12 md:h-12 text-lime-500 animate-spin mx-auto mb-4" />
          <p className="text-slate-400 text-sm md:text-base">Синхронизация с облаком...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-lime-500 selection:text-slate-950 flex flex-col pb-20 md:pb-8 relative">
      ...
      {/* Пользовательская модалка подтверждения (вместо window.confirm) */}
      {bikeToDelete && (
    ...
    <div className="space-y-6">
      <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-xl">
        <div className="h-40 md:h-48 w-full relative">
          ...
          <img src={bike.photo} loading="lazy" alt={bike.name} className="w-full h-full object-cover opacity-60" />
          <div className="absolute bottom-4 left-4 md:left-6 z-20 pr-4">
            ...
            <p className="text-slate-300 text-xs md:text-sm">{bike.model} • Общий пробег: <span className="text-lime-400 font-bold">{Math.round(bike.totalMileage ?? 0)} км</span></p>
          </div>
        </div>
      ...
      {alerts.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="grid sm:grid-cols-2 gap-3 md:gap-4">
            {alerts.map(alert => (
              <div key={alert.id} className={`p-4 rounded-xl border flex flex-col justify-between ${alert.status === 'critical' ? 'bg-red-500/10 border-red-500/30' : 'bg-orange-500/10 border-orange-500/30'}`}>
                ...
                <div className="text-xs font-mono text-slate-400">
                  {alert.remainingKm > 0 
                    ? `Осталось: ~${alert.remainingKm} км (хватит на ~${alert.predictedDays} дней)` 
                    : `Перепробег: ${Math.abs(alert.remainingKm)} км`}
                </div>
              </div>
            ))}
          </div>
  ...
  useEffect(() => {
    const runAudit = async () => {
      try {
        ...
        Общий пробег: ${Math.round(bike.totalMileage ?? 0)} км.
        Износ ключевых узлов:
        ${components.filter(c => c.lifespan > 0).map(c => `- ${c.name}: износ ${Math.round(( ((bike.totalMileage ?? 0) - c.installMileage) / c.lifespan ) * 100)}%`).join('\n')}
...
// ПЕРЕПИСАННАЯ ВКЛАДКА "УЗЛЫ" С МОБИЛЬНОЙ КАРТОЧНОЙ ВЕРСТКОЙ
function ComponentsTab({ components, onReplace, onAddNew }) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      {/* МОБИЛЬНАЯ / КАРТОЧНАЯ ВЕРСТКА ВМЕСТО ТАБЛИЦЫ */}
      <div className="space-y-6">
                       ...
                       {comp.lifespan > 0 ? (
                         <>
                           ...
                           <div className="text-[10px] text-slate-500 mt-1 flex items-center gap-1">
                             <Info className="w-3 h-3" /> Осталось: ~{comp.remainingKm} км ({comp.predictedDays} дн.)
                           </div>
                         </>
                       ) : (
          ...
          {bikes.map(bike => (
            <div key={bike.id} className={`bg-slate-900 border rounded-2xl overflow-hidden shadow-xl transition-all ${activeBikeId === bike.id ? 'border-lime-500 ring-1 ring-lime-500' : 'border-slate-800 hover:border-slate-700'}`}>
              <div className="h-36 md:h-40 w-full relative cursor-pointer group" onClick={() => onSelectBike(bike.id)}>
                ...
                <img src={bike.photo} loading="lazy" alt={bike.name} className="w-full h-full object-cover opacity-70 group-hover:scale-105 transition-transform duration-500" />
              ...
              <div className="p-4 md:p-5 flex justify-between items-center bg-slate-900">
                <div>
                  ...
                  <div className="text-lg md:text-xl font-bold text-slate-200">{Math.round(bike.totalMileage ?? 0)} км</div>
                </div>
...
function AddBikeModal({ onClose, onSave }) {
  ...
  const handleParseSpecs = (e) => {
    ...
    const addComp = (cat, type, name, ls) => {
      parsedComponents.push({ category: cat, type: type, name: name, lifespan: ls, currentWornMileage: 0, id: generateId() });
    };
  ...
  const handleAiParse = async () => {
    ...
    try {
      const prompt = `You are an Expert Bicycle Data Architect and Master Mechanic.
      Analyze the provided bicycle specification text and extract both the bicycle's metadata and its components into a structured JSON.
      
      CRITICAL ARCHITECTURAL RULES:
      1. METADATA: Extract Brand, Model, Year, and Category (MTB, Road, City/Hybrid, Gravel, etc.) if mentioned.
      2. SPLIT PAIRS: Bicycles wear unevenly. You MUST split paired consumable items into TWO separate components: "Front Tire" & "Rear Tire", "Front Brake Pads" & "Rear Brake Pads", "Front Rotor" & "Rear Rotor".
      3. TAXONOMY: Use EXACT categories for components: "Frame & Fork", "Drivetrain", "Wheelset", "Braking System", "Cockpit", "Accessories".
      4. STANDARDS & INTERFACES: If the text mentions compatibility standards (e.g., BSA 68mm, Tapered, Centerlock, BCD 104, 700x40c, HG Freehub), put them in the "article" field.
      5. LIFESPAN ESTIMATES (in km): Assign strict estimated lifespans based on component type and material. 
         - Frame: 50000, Fork: 1500 (service interval)
         - Chain: 2000, Cassette/Freewheel: 6000, Crankset: 15000
         - Front Tire: 6000, Rear Tire: 3500
         - Brake Pads (Front/Rear): 1500, Rotors: 8000
         - Cables/Housing: 3000
      6. FOCUS ON CONSUMABLES: Ensure Chain, Cassette, Tires, and Brake Pads are ALWAYS extracted. Keep component names exact (e.g., "Shimano Tourney RD-TY300", "Schwalbe G-One").
      
      Provide all string values (category, type, name, article) in Russian language.`;
      ...
      if (result && result.components && Array.isArray(result.components) && result.components.length > 0) {
        const mapped = result.components.map(comp => ({
          id: generateId(),
        ...
        let generatedBikeName = bikeName;
        if (result.metadata?.brand || result.metadata?.model) {
            generatedBikeName = `${result.metadata.brand || ''} ${result.metadata.model || ''}`.trim();
        }
        if (!generatedBikeName) generatedBikeName = "Мой велосипед";
        
        setParsedData({ 
          name: bikeName !== '' ? bikeName : generatedBikeName, 
          type: result.metadata?.category || "AI Custom Spec", 
          photo: "https://images.unsplash.com/photo-1532298229144-0ec0c57515c7?auto=format&fit=crop&q=80&w=800", 
          components: mapped 
        });
        setStep(3);
      ...
      alert('Ошибка ИИ-анализа. Попробуйте пропустить и добавить узлы вручную.');
      console.error(error);
    } finally {
  ...
  const handleFinalSave = () => {
    ...
    const newComponents = parsedData.components.map(comp => {
      ...
      const calculatedInstallMileage = Math.max(0, baseMileage - wear);
                ...
                <div key={comp.id} className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col md:flex-row gap-3 md:gap-4 md:items-center">
                  <div className="flex-1">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm focus:border-lime-500 focus:outline-none font-bold mb-2"
                      placeholder="Название детали"
                    />
                    <input 
                      type="text" value={comp.article}
                      onChange={(e) => handleUpdateComponent(comp.id, 'article', e.target.value)}
                      className="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-3 py-1.5 text-indigo-300 font-mono text-[10px] md:text-xs focus:border-indigo-500 focus:outline-none"
                      placeholder="Стандарт, артикул или ширина (напр. 700x40c)"
                    />
                  </div>
